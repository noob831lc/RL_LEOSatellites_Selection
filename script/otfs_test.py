...
for i in range(len(channel_delay)):
    r += np.roll(s * np.exp(-1j * 2 * np.pi * channel_doppler[i] / (M * N) * np.arange(1, M * N + 1)),
                 -channel_delay[i])
...
def plot_3d(data, time_axis, title):
    fig = plt.figure(figsize=(10, 8))
    ax = fig.add_subplot(111, projection='3d')
    ax.plot(time_axis, np.abs(data) * np.cos(np.angle(data)), np.abs(data) * np.sin(np.angle(data)), marker='o',
