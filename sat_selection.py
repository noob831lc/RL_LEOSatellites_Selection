...
from utils.leomodel import Satellite, update_satellite_info
from algorithm.selection import ossa_selection, maxele_selection, fcsdp_selection, compute_dgdop_pos
...
if __name__ == '__main__':
    # 基础设置与 TLE 数据加载
    t = datetime(2025, 1, 5, 14, 0, 0, tzinfo=timezone.utc)
    tle_file = 'satellite_data/starlink_tle_20250106_144506.txt'
    satellites_tle = parse_tle_data(tle_file)

    # 定义观测者
    terminal = observer(45.75, 126.68, elevation_m=100)
    user_pos = terminal.at(ts.from_datetime(t)).position.m  # 单位 m

    satellites_sel = [Satellite(sat) for sat in satellites_tle]
    satellites_sel = [sat for sat in satellites_sel if visable(sat.sat_object, terminal, t)]
    print(f"可见卫星数量: {len(satellites_sel)}")
    for sat in satellites_sel:
        update_satellite_info(sat, terminal, t)
    ...
    # ------------------------------
    # RL_GNN 测试部分：加载训练好的模型并进行测试
    # ------------------------------
    m_rl = len(satellites_sel)

    # 构造候选卫星的特征与位置（单位均为 m 或弧度）
    feature_dim = 2
    candidate_positions = np.empty((m_rl, 3))
    candidate_features = np.empty((m_rl, feature_dim))
    for i, sat in enumerate(satellites_sel):
        t_ = ts.from_datetime(t)
        candidate_positions[i] = (sat.sat_object).at(t_).position.m
        diff = (sat.sat_object) - terminal
        topocentric = diff.at(t_)
    ...
    with torch.no_grad():
        ...
        rl_names = [satellites_sel[j].name for j in selected_indices]
        sel_positions_rl = [candidate_positions[j] for j in selected_indices]
        rl_dop = compute_dgdop_pos(sel_positions_rl, user_pos)
